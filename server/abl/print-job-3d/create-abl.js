...
const printJob3DSchema = {
    ...
    required: ['name', 'filamentCodeName', 'customerName', 'totalPrice', 'deliveryDue', 'stateId'],
...
const createAbl = async (req, res) => {
    try {
        ...
        if (new Date(printJob3DDtoIn.deliveryDue).setHours(0, 0, 0, 0) < new Date().setHours(0, 0, 0, 0)) {
